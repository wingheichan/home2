...
(async function(){
  ...
  document.getElementById('quizPreview').addEventListener('click', ()=>{
    ...
    if(!list.length){ showPreviewOverlay('Quiz Preview','<p>No items.</p>'); return; }
    const html = list.map((it,idx)=>`<div style="margin:8px 0"><strong>Q${idx+1}.</strong> ${it.q}<br><em>Answer:</em> ${it.choices[it.a]}</div>`).join('');
    showPreviewOverlay(`Quiz Preview — ${selCat.value} / ${selSub.value}`, html);
  });
